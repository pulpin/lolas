    ...
    public partial class ConsultaCuentaCorriente : Form
    {
        ...
        private void cargar()
        {
           // double supago=0,totalapagar=0, porcen;
            ...
            gConsulta.DataSource = cli.Mostrar_detallecuentacorriente();
           // gConsulta2.DataSource = ren.Mostrar_rendicionesdetallesagrupados();
           /* for (int i = 0; i < gridViewPintarFilas.DataRowCount; i++)
            {
               // importe += Convert.ToDouble(gridViewPintarFilas.GetRowCellValue(i, "CUCO_APAGAR").ToString());
               // supago += Convert.ToDouble(gridViewPintarFilas.GetRowCellValue(i, "CUCO_PAGO").ToString());
              //  totalapagar += Convert.ToDouble(gridViewPintarFilas.GetRowCellValue(i, "CUCO_SALDO").ToString());
            }
            */
            /*
        ...
        public void pl_CreateReportHeaderArea2(object sender, CreateAreaEventArgs e)
        {
            ...
            planilla = "Planilla de rendición de consigna";
           // edi = "Señores:  ";
           // dom = "Domicilio: ";
          //  tel = "teléfono: ";
            ...
            TextBrick brick7 = e.Graph.DrawString("Señores: "+senores, Color.Black,
        ...
        {
            if (Convert.ToString(this.gridViewPintarFilas.GetRowCellValue(gridViewPintarFilas.FocusedRowHandle, this.gridViewPintarFilas.Columns["CUCO_SALDO"])) != "0,00")
            {
                if (Globales.puntodeventa == 0)
                { 
                    Elegirpto ele = new Elegirpto();
                    ele.ShowDialog();
                }
                PagarCC aa = new PagarCC();
                aa.Importe = Convert.ToString(this.gridViewPintarFilas.GetRowCellValue(gridViewPintarFilas.FocusedRowHandle, this.gridViewPintarFilas.Columns["CUCO_SALDO"]));
                aa.Saldoactual = Convert.ToString(this.gridViewPintarFilas.GetRowCellValue(gridViewPintarFilas.FocusedRowHandle, this.gridViewPintarFilas.Columns["CUCO_PAGO"])); 
                aa.Ide = Convert.ToInt32(this.gridViewPintarFilas.GetRowCellValue(gridViewPintarFilas.FocusedRowHandle, this.gridViewPintarFilas.Columns["CUCO_IDE"]));
                aa.colocarvalores();
