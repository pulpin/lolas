...
{
    public partial class AltaDineroCaja : Form
    {
        ...
        private void Altadeposito()
        {
            ...
            dc.Precio = txtprecio.Text;
            dc.Obs = txtobs.Text;
            int resultado = dc.spAltaDineroCaja();
            if (resultado == 0)
            {
                this.DialogResult = System.Windows.Forms.DialogResult.OK;
                MessageBox.Show("Se ha guardado el dinero de la CAJA!");
            }
            else if (resultado == 1)
            {
                MessageBox.Show("Ya se encuentra cargada la caja del día");
            }
            else
            {
        ...
        private void Modificardeposito()
        {
            ...
            de.Codigo = this.Codigo;
            de.Depoide = this.Ide;
            de.Alta = this.Alta;
            de.Habia = this.Habia;
            int resultado = de.spAgregarDeposito();
            if (resultado == 0)
            {
                this.DialogResult = System.Windows.Forms.DialogResult.OK;
                MessageBox.Show("Se ha modificado el stock en el depósito!");
            }
            else
            {
                MessageBox.Show("Se ha producido un error!");
